...
class _AIChatScreenState extends State<AIChatScreen> {
  ...
  Future<void> _attachDocument() async {
    ...
    if (choice == kFile) {
      ...
      context.read<ChatProvider>().sendDocument(
            File(pf.path!),
            pf.name,
            isImage: isImage,
          );
  ...
  Widget build(BuildContext context) {
    ...
    final inputEnabled = chat.state == ChatState.collectingSymptoms ||
        chat.state == ChatState.collectingBookingPrefs;

    final attachEnabled = chat.state == ChatState.collectingSymptoms ||
  ...
  Widget _buildSpecial(ChatMessageModel msg) {
    switch (msg.type) {
      case ChatMessageType.bookingPrompt:
        return _buildBookingPrompt();
          ...
          Flexible(
            child: _card(
              child: Column(
                crossAxisAlignment: CrossAxisAlignment.start,
                children: [
                  const Text('Would you like to book an appointment?',
                      style: TextStyle(
                          fontSize: 15,
                          fontWeight: FontWeight.w600,
                          color: AppColors.neutral800)),
                  ...
                  HoverPrimaryButton(
                    ...
                    child: SizedBox(
                      ...
                      child: ElevatedButton.icon(
                        onPressed: chat.acceptBooking,
                        icon: const Icon(Icons.check, size: 18),
                        label: const Text('Yes, book now'),
                        style: ElevatedButton.styleFrom(
                          backgroundColor: AppColors.primary600,
                          foregroundColor: Colors.white,
                  ...
                  const SizedBox(height: 8),
                  Center(
                    child: HoverSecondaryButton(
                      scale: 0.98,
                      child: TextButton(
                        onPressed: chat.declineBooking,
